...
Original file is located at
    https://colab.research.google.com/drive/1l70EppLPwEAblME_1tWH4KI0fDxSfbrF
"""
...
def solve_square(a, b, c):
    ...
    def coef_str(aF):
        if aF == 1: return ""
        if aF == -1: return "-"
        return frac_to_str(aF)

    if h == 0:
        square = f"{coef_str(aF)}x^2"
    else:
        inner = f"x - {frac_to_str(h)}" if h > 0 else f"x + {frac_to_str(-h)}"
        square = f"{coef_str(aF)}({inner})^2"

    if k == 0:
        tail = ""
    elif k > 0:
        tail = f" + {frac_to_str(k)}"
    else:
        tail = f" - {frac_to_str(-k)}"
    ...
    print("Completed square form:", expr)
    print("Vertex (h, k):", vertex)
# Show result
...
def algetool():
    ...
    print("7. Complete the Square (a(x - h) + k)")
